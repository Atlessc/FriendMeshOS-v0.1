...
"${CXX:-c++}" -std=c++17 -Wall -Wextra -Werror -pedantic -fsanitize=undefined,bounds \
    ...
    "$ROOT/src/friendmesh/storage/FriendMeshDeviceBinding.cpp" \
    "$ROOT/src/friendmesh/storage/FriendMeshMasterKey.cpp" \
    "$ROOT/src/friendmesh/storage/FriendMeshStorageKeyManager.cpp" \
    "$ROOT/src/friendmesh/storage/FriendMeshWrappedKeyStore.cpp" \
