...
| FMD-002 | Versioned signed `FriendMeshEnvelope` dispatch on `PRIVATE_APP` | New protobuf/module/service | Standard ports and opaque relay compatibility | VERIFYING | Phase 2 foundation committed at `0d45a16`; outbound/fuzz follow-up in working tree | Generated 229-byte maximum frame; strict receive/outbound/replay checks and 150,000-case sanitizer corpus pass; physical opaque-relay/send proof pending | Revisit if upstream assigns a registered port or changes private-app handling |
| FMD-003 | FriendMesh Ed25519 signing identity bound to Meshtastic PKI identity | Identity/security service and protected storage | Existing X25519/PKI DM behavior and reset warnings | VERIFYING | Phase 2 foundation committed at `0d45a16`; lifecycle boundary follow-up in working tree | RFC 8032 vectors and ESP32 libsodium compile/link pass; protected-store-only lifecycle and NodeDB binding exist; actual protected persistence, verification UI, and physical two-device proof pending | Never replace or derive from upstream X25519 key bytes; never add a plaintext convenience store |
